...
| `FFmpeg/FFprobe missing` banner in AppImage              | Bundled tools were present under `resources/ffmpeg/` but the app resolves `$RESOURCE/ffmpeg/`. Rebuild with `./scripts/build-appimage.sh` (normalizes layout into `usr/lib/replaybox/ffmpeg/` and asserts `-version` before/after re-pack)                                                                                                              |
